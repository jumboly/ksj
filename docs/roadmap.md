...
**成果物**:
- `integrator/schema_unify.py` — カラム型・順序の統一
- 逐次結合による大容量対応 (`pyarrow.RecordBatch` ストリーム)
- `--allow-partial` フラグ
- CRS 変換 (pyproj): JGD2011/JGD2000/Tokyo Datum ⇄ target-crs
- 欠損値コードの NaN 正規化
- 出力メタデータに coverage と欠落リストを記録

**動作確認**:
```bash
uv run ksj integrate L03-a --year 2021               # mesh3、数百ファイル結合
uv run ksj integrate L03-a --year 1976               # 旧測地系 → JGD2011 変換
uv run ksj integrate A03 --year 2003                 # urban_area (partial) → 警告
uv run ksj integrate A03 --year 2003 --allow-partial # 続行
