...
src/ksj/
  ...
  cli.py                   # typer サブコマンド定義 (薄い、handlers/renderers にディスパッチ)
  errors.py                # HandlerError / ErrorKind enum
  html_cache.py            # KSJ 詳細ページのローカルキャッシュ (refresh の入力)
  handlers/                # 純粋関数層: 入出力を dataclass でやり取り (MCP/Web API 流用可)
  renderers/               # 出力整形: rich_render.py (人間向け Table) / json_render.py (機械向け)
  catalog/
    __init__.py
    schema.py              # pydantic モデル: Catalog / Dataset / Version / FileEntry
                           #   + UseCase Literal (license_raw は文字列のまま保持)
    loader.py              # YAML 読込・検証、annotations.yaml の merge
    refresh.py             # ksj catalog refresh: HTML スクレイパ、annotations 欠損検知
    _parser.py             # 詳細ページの thead/tbody パーサ (BeautifulSoup)
    _normalizers.py        # CRS / format / scope / geometry_types の正規化
  downloader/
    __init__.py
...
catalog/                   # プロジェクトルート直下 (パッケージ外)
  datasets.yaml            # scraper 管理 (ksj catalog refresh が上書き)
  annotations.yaml         # 人手 + LLM 管理 (description / use_cases、refresh は触らない)
```
...
| レイヤ | 責務 | 外部依存 |
|---|---|---|
| `cli` | 引数解析、サブコマンド分岐、handlers/renderers へのディスパッチ | typer |
| `handlers` | 純粋関数のコマンド実装。dataclass で結果を返し MCP / Web API で流用可 | — (本体 stdlib + pydantic) |
| `renderers` | 出力整形。rich (人間向け Table / progress) と JSON (機械向け `{ok, data}` 契約) を分離 | rich |
| `catalog` | データセットメタ・URL・CRS・形式・使用許諾条件原文 (license_raw) の管理と再スクレイプ。datasets.yaml + annotations.yaml の merge も担当 | pydantic, pyyaml, beautifulsoup4, lxml, httpx |
...
├── tests/
├── catalog/
│   ├── datasets.yaml           scraper 管理
│   └── annotations.yaml        description / use_cases (scraper 非触)
├── docs/                       本ディレクトリ (仕様書)
└── data/                       (gitignore)
    ├── raw/<code>/<year>/*.zip
