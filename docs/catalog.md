...
| `license_raw` | str | — | 詳細ページ「使用許諾条件」欄の原文 (全文、truncate なし)。年度別分岐や県別制限を含む文面が多く、機械分類は誤判定が多いため構造化せず原文のまま保持する | datasets.yaml |
| `available_formats` | list[Format] | — | ページ全体で宣言された利用可能 format の union。`FileEntry.format = multi` の内訳を参照する先として使う (KSJ 慣行として `_GML.zip` 1 本に宣言 format が全て同梱されるため、どの format が取り出せるかは dataset 単位の宣言から確定できる) | datasets.yaml |
...
|---|---|---|---|
| `national` | なし | — | 全国一括 |
| `region` | `region` | — | 日本語名 (「東北地方」「甲信越・北陸地方」等)。HTML 原文をそのまま保持。DOM id fallback 時のみ 2 桁数値 (`51`-`59`) が入る |
| `regional_bureau` | `bureau` | — | 日本語名 (「北海道開発局」「東北地方整備局」等)。HTML 原文をそのまま保持。DOM id fallback 時のみ 2 桁数値 (`81`-`89`) が入る |
| `prefecture` | `pref_code` | `pref_name` | 1-47 の int |
| `urban_area` | `urban_area` | — | `首都圏` / `関東圏` / `中部圏` / `近畿圏` (HTML 原文をそのまま保持、filename fallback 時のみ `SYUTO` / `CHUBU` / `KINKI`) |
| `river` | `river` | — | 河川名または ID (現カタログ未出現、HTML 原文保持) |
| `municipality` | `municipality` | — | 市町村名または 5 桁コード (現カタログ未出現、HTML 原文保持) |
| `mesh1`-`mesh6` | `mesh_code` | — | 下の「scope 語彙」表参照 |
| `special` | `special` | — | 上記に当てはまらない例外。HTML 原文をそのまま保持 |
...
| `region` | `region` (日本語名、DOM id fallback 時のみ 51-59 数値) | 地方区分 (北海道/東北/関東/中部/近畿/中国/四国/九州/沖縄) | L01, W05, N03 |
| `regional_bureau` | `bureau` (日本語名) | 北海道開発局 + 地方整備局 | A53, A31a |
| `prefecture` | `pref_code` (1-47), `pref_name` | 都道府県 | 多数 |
| `urban_area` | `urban_area` (関東圏/中部圏/近畿圏 等。KSJ HTML 原文を保持) | 三大都市圏 | A03, L03-b-u, L03-b-c |
| `river` | `river` | 一級河川単位 | (現状出現なし) |
| `municipality` | `municipality` | 市町村単位 | A51 の一部 |
...
| `special` | `special` | 上記に該当しない特殊単位 | (現状出現なし) |

メッシュの桁→次数は JIS X 0410 準拠。KSJ の配布は原則 1 次メッシュ単位 (4桁コード) で ZIP 化されており、内部データは 100m / 250m 等の細分メッシュでも ZIP スコープは `mesh1` になる。

### 地方整備局と filename 数値コードの対応

KSJ の URL 末尾には数値コード (例: `A53-23-82_GML.zip` の `82`) が使われる。HTML の「地域」列は通常日本語名で記載されるため、`bureau` フィールドには原文をそのまま保存する (数値への alias はしない)。参考対応表:
...
URL の末尾コードで実証済み。沖縄総合事務局は現カタログに出現せずコード未確認 (出現を検知したら追加する運用)。DOM id fallback (`prefecture81` 等、HTML text が空の場合) では `bureau` に数値コードが入る。
...
`license_raw` は KSJ 詳細ページ「使用許諾条件」欄の原文をそのまま保持する。構造化分類 (LicenseProfile) は Phase 9 で一度導入したが以下の理由で撤回した:

- 年度別分岐 (「2019年以降：CC_BY_4.0 / 上記以外：非商用」) の判定条件がアドホックになる (「整備年度」「作成年度」のような属性的年度を除外する規則、閾値 regex、`上記以外` sentinel 等、判定ロジックが実測文面に強く依存する)
- 県別制限や「申請等必要」等の細則は自由文で書かれており機械判定が効かない
- KSJ 側の文言変更で再現性が崩れる (タイポ「CC_B.Y_4.0」のような個別対応が増える)

採用方針: 原文を正とし、解釈 (商用可否判定・年度分岐の展開・推薦時のフィルタ) は利用側 (LLM / 人間レビュー) に委ねる。将来「商用可否」等の派生属性が本当に必要になったら Version 単位で手動付与する選択肢を残す (Phase 10 以降で再検討)。

`source_terms_url` に相当する利用規約は一律 https://nlftp.mlit.go.jp/ksj/other/agreement.html なので別途参照する。
...
`multi` は「ページに複数形式の記載があり、かつ filename 末尾から単一形式に絞り込めない」場合の値。KSJ の `_GML.zip` は慣行的に ZIP 内部に Shapefile も同梱するため、形式宣言が 2 個以上かつ filename が `_GML.zip` なら `multi` とする。現カタログでは全エントリの 85% が `multi` に倒れるため、「この ZIP に具体的にどの format が入るか」は **`Dataset.available_formats` を参照する**のが正しい (ページ宣言 format の union が格納されている)。
...
datasets:
  ...
  N03:
    ...
    available_formats: [gml_jpgis2014, gml_jpgis21, shp, geojson]
    versions:
      "2025":
        files:
          - scope: national
...
2. 無ければ識別子 (`pref_code` / `mesh_code` / `bureau` / `urban_area` / `region` / `municipality` / `river`) ごとに対象年度以前で最新の 1 件を選び union
