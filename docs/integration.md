...
8. **結合**: `GeoDataFrame.pd.concat` で結合
9. **出力**: `pyogrio.write_dataframe` で GeoPackage の 1 レイヤに出力、`gpkg_metadata` テーブルに出典 JSON を埋込
...
- カラム型: pandas 自動昇格に任せる。Int64 と float が混ざれば float64、
  float と str が混ざれば object になる (explicit な int→float→str 制御は行わない)
- 欠損カラムは NaN で埋める
- カラム順序: 統合時に一定の順序で並べ直す (カタログ定義順 or 辞書順)
- 属性フィールド名の英語化は**行わない**

実装は `integrator/schema_unify.py` の `unify()` 関数。source_year 列を concat 前に
各フレームへ付与 → null_values を列 dtype ごとに NaN 化 → `pd.concat(join="outer")`
という流れ。将来フェーズで GeoPackage の append 書出によるストリーミング結合や DBF 254 バイト制限
対応 (A46/A47/A48 等の分割された住所列の結合) を足す予定。

## 属性正規化の範囲

- **UTF-8 化**: Shift_JIS 配布のデータを UTF-8 に統一。
  pipeline 側で format=shp なら ``encoding="cp932"`` を pyogrio に渡し、GML/GeoJSON は
  仕様上 UTF-8 が保証されるので pyogrio のデフォルト判定に任せる。カタログの
  ``FileEntry.encoding`` を埋めれば手動で override できる (例外的な UTF-8 Shapefile への対応)
- **スキーマ統一**: 分割ファイル間のカラム型・順序を揃える
- **欠損値正規化**: 独自欠損値コード (`-999` 等) を NaN に。
  数値の null は数値列、文字列の null は文字列列にのみ適用 (意図しない置換を避けるため)
- **DBF 254 バイト制限への対処** (A46/A47/A48 等): 住所が別カラムに分割されているケースで結合ルールを定義 (Phase 7 以降)

## 出力メタデータ

GeoPackage の `gpkg_metadata` テーブル (MIME `application/json`) に以下を埋め込む:
...
```json
{
  ...
  "license": { /* LicenseProfile (kind/commercial_use/attribution_required/derivative_works/constraints/by_year)。スキーマは catalog.md 参照 */ },
  ...
  "ksj_tool_version": "0.2.0"
}
```

`license` は `Dataset.license` (LicenseProfile) を `model_dump(mode="json")` で展開した dict。スキーマ詳細と分類表は [`catalog.md#licenseprofile-の正規化`](catalog.md#licenseprofile-の正規化) を参照。

`mixed_by_year` のデータセットでは `by_year` に `{"2019": {...}, "_else": {...}}` のような子プロファイルが入る。利用側は `commercial_use` を採用用途の可否判定に使い、`_else` は「条件に当てはまらない年度」のデフォルト扱い。
