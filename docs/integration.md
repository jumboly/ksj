...
3. national が無いとき:
   - ファイルを `scope` + 識別子 (`pref_code` / `mesh_code` / `bureau` /
     `urban_area` / `region` / `municipality` / `river`) でバケット化
...
```json
{
  ...
  "license": "オープンデータ（CC_BY_4.0） ※本データを二次利用する場合には、国土地理院に申請等必要な場合があります。",
...
`license` は `Dataset.license_raw` (KSJ 詳細ページ「使用許諾条件」欄の原文) をそのまま埋め込む。商用可否や年度別分岐の判定は利用側 (LLM / 人間レビュー) に委ねる (撤回の経緯は [`catalog.md#ライセンス-license_raw`](catalog.md#ライセンス-license_raw) 参照)。
